...
export const TrackListCard = ({ track, index }: { track: Track; index: number }) => {
  ...
  const testimonials =
    track.photos && track.photos.length > 0
      ? track.photos.map((photo) => ({ src: photo.url }))
      : [
        {
          src: track.coverUrl,
        },
      ];
  console.log(testimonials, "lista de fotos ")
  ...
  return (
    ...
    >
      ...
      <div
        className={`track-info-area bg-white/10 p-6 md:p-8  flex flex-col w-full md:w-1/3 justify-center md:justify-between transition-all duration-700 ${isEven ? 'md:self-end md:rounded-r-[2rem]' : 'md:self-start md:rounded-l-[2rem]'
          }`}
      ...
      >
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            <div
              className={`h-full rounded-full transition-all duration-200 ${isThisTrackPlaying ? 'bg-primary' : 'bg-transparent'
                }`}
              style={{
                width:
